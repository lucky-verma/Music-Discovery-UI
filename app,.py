...
class MusicDownloader:
    ...
    def download_single_song(self, url, artist=None, album=None):
        """Download a single song using ytdl-sub"""
        try:
            ...
            subscription = {
                'temp_download': {
                    ...
                    'overrides': {
                        'music_directory': self.music_path,
                        'url': url
                    }
                }
            }
...
def main():
    ...
    with st.sidebar:
        ...
        try:
            music_files = subprocess.run(
                ["find", "/music", "-name", "*.mp3", "-o", "-name", "*.m4a", "-o", "-name", "*.flac"],
        ...
        youtube_liked = st.text_input("Liked Songs Playlist ID", placeholder="LM or your liked songs ID")
        ...
        if st.button("💾 Save Automation Settings"):
            ...
            automation_config = {
                'automation': {
                    ...
                    'spotify_playlists': spotify_playlists.split('\n') if spotify_playlists else [],
    ...
    # Main content area
    tab1, tab2, tab3, tab4 = st.tabs(["🎵 Quick Download", "📋 Playlist Manager", "🔍 Discovery", "📊 Download History"])
    ...
    with tab1:
        ...
        with col1:
            ...
            if url and not downloader.validate_url(url):
                st.error("❌ Invalid URL format. Please use YouTube or YouTube Music URLs.")
            
            # Optional metadata
            col_artist, col_album = st.columns(2)
            with col_artist:
                artist_override = st.text_input("🎤 Artist (optional)", placeholder="Override artist name")
            with col_album:
                album_override = st.text_input("💿 Album (optional)", placeholder="Override album name")
    ...
    with tab2:
        ...
        with col1:
            if st.button("📥 Download Playlist", type="primary", disabled=not playlist_url):
                if playlist_url:
                    with st.spinner("⏳ Downloading playlist... This may take 10-30 minutes"):
                        success, stdout, stderr = downloader.download_playlist(playlist_url, playlist_name)
    ...
    with tab3:
        ...
        search_query = st.text_input("🔎 Search YouTube Music:", placeholder="Enter artist, song, or album name")
                        ...
                        if result.returncode == 0:
                            ...
                            if results:
                                ...
                                for i, result in enumerate(results):
                                    ...
                                    with col2:
                                        if st.button(f"📥", key=f"download_{i}", help="Download this song"):
                                            # Quick download
                                            with st.spinner("Downloading..."):
                                                success, _, _ = downloader.download_single_song(result['url'])
                                                if success:
        ...
        with col1:
            ...
            if st.button(f"🔍 Search {selected_genre}"):
                st.experimental_set_query_params(search=f"{selected_genre} music 2024")
        
        with col2:
            st.markdown("**🌍 Regional Music:**")
            regions = ["Bollywood", "K-Pop", "J-Pop", "Latin", "Arabic", "African", "European"]
            selected_region = st.selectbox("Browse by region:", regions)
            if st.button(f"🔍 Search {selected_region}"):
                st.experimental_set_query_params(search=f"{selected_region} music hits")
        ...
        with col3:
            ...
            if st.button(f"🔍 Search {selected_period}"):
                st.experimental_set_query_params(search=f"best songs {selected_period}")
